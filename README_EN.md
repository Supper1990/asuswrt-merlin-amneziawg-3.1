...
An idempotent AdGuard Home integration is also included. When port 53 is actually owned by `AdGuardHome`, the add-on reads the active YAML path from the process `-c` argument, preserves domain-specific rules, bootstrap and fallback resolvers, and directs the ordinary upstream to `127.0.0.1:<dnsmasq port>`. Existing loopback targets in domain and local PTR rules are moved to the same port. A correct configuration is neither rewritten nor restarted. For a required change, AGH is stopped and started through its init script; failure to verify the new start restores the original YAML. The check runs during Apply, start, page installation, watchdog and firewall recovery. Local tests pass; the AGH mutation path still requires Merlin validation.
