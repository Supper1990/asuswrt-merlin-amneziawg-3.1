...
В `awg_dst` загружаются только GeoIP-категории, выбранные в поле **GeoIP Service Lists**; снятые категории удаляются при применении настроек. GeoSite-категории разбираются из актуального формата `dlc.dat_plain.yml`, а очищенные GeoSite и Custom Domains больше не оставляют старых правил.
